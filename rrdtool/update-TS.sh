...
VALVE="$(  echo "$OUTPUT" | grep "valve"      | sed 's/^[^=]* = //' | sed 's/off/0/;s/on/1/' )"
BOOST="$(  echo "$OUTPUT" | grep "boost"      | sed 's/^[^=]* = //' | sed 's/off/0/;s/on/1/' )"
DAY="$(    echo "$OUTPUT" | grep "day"        | sed 's/^[^=]* = //' | sed 's/off/0/;s/on/1/' )"
PUMP="$(   echo "$OUTPUT" | grep "pump"       | sed 's/^[^=]* = //' | sed 's/off/0/;s/on/1/' )"
...
rrdtool update "$FILENAME" --template \
	...
	"N:${TEMP:-U}:${TTEMP:-U}:${VALVE:-U}:${BOOST:-U}:${DAY:-U}:${COOLER:-U}:${LOW:-U}:${HIGH:-U}"
